...
/*!50001 VIEW `ending_inventory_view` AS select `pv`.`product_id` AS `product_id`,`pv`.`variant_id` AS `variant_id`,sum(`pv`.`quantity`) AS `ending_inventory` from `productvariant` `pv` where (`pv`.`date_added` <= '2024-06-11') group by `pv`.`product_id`,`pv`.`variant_id` */;
